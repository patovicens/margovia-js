...
Use `completeRun` only when a cost event is also the terminal event for the run. Tracked adapters, provider helpers, and wrapped provider calls use this internally for one-call workflows. For multi-step workflows, complete the run after all steps finish.
