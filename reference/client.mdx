...
Installing the SDK alone sends nothing. Without `MARGOVIA_API_KEY`, the client is a no-op. With `MARGOVIA_API_KEY`, it sends to Margovia Cloud unless `baseUrl` or `MARGOVIA_BASE_URL` points at a self-hosted or compatible receiver.
...
Creates a tracked OpenAI adapter. The adapter starts a run, calls `client.chat.completions.create(request)`, records cost from the response usage, and completes or fails the run.
...
Creates a tracked Anthropic adapter. The adapter starts a run, calls `client.messages.create(request)`, records cost from the response usage, and completes or fails the run.
