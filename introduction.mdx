...
The JavaScript SDK is the instrumentation layer. Install it in the backend that already calls OpenAI, Anthropic, or another model provider, then send usage and attribution events to Margovia.
...
Use the SDK in server-side code only:
...
Do not use the SDK in browser code. `MARGOVIA_API_KEY` is a server-side secret.
