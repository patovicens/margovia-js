...
For normal OpenAI or Anthropic calls, prefer `wrapOpenAI(...)`, `wrapAnthropic(...)`, `trackOpenAI(...)`, or `trackAnthropic(...)`. Those APIs can read provider usage and report cost automatically.
