...
The adapter starts a run, calls `client.messages.create(request)`, reads `response.usage`, records the cost event, and completes or fails the run.
...
This is useful for wide migrations, but the tracked adapter is clearer for new code.
